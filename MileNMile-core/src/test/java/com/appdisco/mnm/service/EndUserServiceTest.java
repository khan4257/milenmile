...
public class EndUserServiceTest {
	...
	public void endUserTest() throws Exception {
		EndUserVO endUserVO = createAccount();
		
		chkPassword(endUserVO.getId(), endUserVO.getUsername(), endUser.getPassword());
		
		String newPwd = "test1";
		updatePassword(endUserVO.getId(), endUser.getPassword(), newPwd);
	...
	public void remove() throws Exception {
		endUserService.deleteAllEndUserInfo(endUser.getId());
	}
	...
	public EndUserVO createAccount() throws Exception {
		endUser = new EndUser();
		endUser.setUsername("endusertest@test.com");
		endUser.setPassword("test");
		endUser.setName("test");
		endUser.setPhone_no("01012341234");
		
		EndUserVO endUserVO = endUserService.createAccount(endUser.getUsername(), endUser.getPassword(), endUser.getName(), endUser.getPhone_no(), 1985, 1, false, false, Locale.KOREAN);
		
		assertTrue(endUserVO.getId() > 0);
		endUser.setId(endUserVO.getId());
		return endUserVO;
	}
