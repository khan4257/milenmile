...
public class EndUserService extends BaseService {
	 ...
	 */
	public EndUserVO createAccount(
			String username, 
			String password, 
			String name, 
			String phone_no, 
		...
		EndUserVO vo = new EndUserVO(endUser, endUserProfile);
	 ...
	 * @throws ApiException the api exception
	 */
	public boolean signOut(String access_id) throws ApiException {
		Long id = getEndUserAccess(access_id).getU_id();
		HashMap<String, Object> params = new HashMap<String, Object>();
		params.put("role", 4);
		params.put("secession_date", new Date());
		return updateEndUser(id, params);
	}
	...
	public Map<String, String> login(String email, String password, String device_id, String push_key, Locale locale) throws ApiException {
		...
		EndUserSession session = createSession(access.getId(), null, locale);
...
//		list.add(session.getId());
	 ...
	 * @throws ApiException the api exception
	 */
	public Map<String, String> login(String access, Integer nfc_id, Locale locale) throws ApiException {
		
		if (!validAccessKey(access, locale)) {
			throw new ApiException(ApiCode.NOT_EXISTS_USER_ERROR);
		}
		
		EndUserSession session = createSession(access, nfc_id, locale);
	...
	public EndUserAccess createEndUserAccess(Long u_id, String device_id, String push_key, Locale locale) throws ApiException {
		...
		try {
			endUserAccessDao.expireAccess(u_id);
	...
	public EndUserSession createSession(String access_id, Integer nfc_id, Locale locale) throws ApiException {
		
		expireSession(access_id, locale);
		
		EndUserSession session = new EndUserSession();
		
		session.setId(IdGenerator.getUUID());
		session.setAccess_id(access_id);
		session.setNfc_id(nfc_id);
	 ...
	 */
	public EndUserProfile createEndUserProfile(Long id, int birth_year, int gender, boolean isMarried, boolean haveCar, Locale locale) throws ApiException {
		...
		endUserProfile.setRecievePush(true);
		endUserProfile.setRecieveSms(true);
	 ...
	 * @throws ApiException the api exception
	 */
	public String updateProfileImage(Long id, MultipartFile image, Locale locale) throws ApiException {
		...
		if (updateEndUserProfile(id, params, locale)) {
			return getRootImageUrl() + filename;
		} else {
...
//		return updateEndUserProfile(id, params, locale);
	...
	public void deleteAllEndUserInfo(String session, String access) throws ApiException {
		EndUserAccess endUserAccess = getEndUserAccess(access);
		
		if (endUserAccess == null)
			return;
		
		Long id = getEndUserAccess(access).getU_id();
		expireSession(access, Locale.KOREAN);
		endUserSessionDao.clearAllExpiredSession();
		endUserAccessDao.delete(access);
		deleteAllEndUserInfo(id);
	}
	
	public void deleteAllEndUserInfo(Long id) throws ApiException {
		endUserProfileDao.delete(id);
		endUserDao.delete(id);
	}
