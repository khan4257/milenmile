...
public class EndUserVO implements Serializable {
	...
	public EndUserVO(EndUser endUser, EndUserProfile endUserProfile) {
		if (endUser != null) {
			id = endUser.getId();
			name = endUser.getName();
			username = endUser.getUsername();
			phone_no = endUser.getPhone_no();
			reg_date = endUser.getReg_date();
		}
		if (endUserProfile != null) {
			gender = endUserProfile.getGender();
			birth_year = endUserProfile.getBirth_year();
			profile = endUserProfile.getProfile();
			profile_img = endUserProfile.getProfile_img();
			job = endUserProfile.getJob();
			loc = endUserProfile.getLoc();
			isMarried = endUserProfile.isMarried();
			haveCar = endUserProfile.getHaveCar();
			recievePush = endUserProfile.getRecievePush();
			recieveSms = endUserProfile.getRecieveSms();
			point = endUserProfile.getPoint();
		}
	}
