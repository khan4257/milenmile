...
public class BaseDaoMybatis<PK, T> extends SqlSessionDaoSupport {
	...
	public T insert(T entity) {
		getSqlSession().insert(getKey("ins"), entity);
		return entity;
	}
	...
	public boolean update(T entity) {
		int updatedRow = getSqlSession().update(getKey("upt"), entity);
		return updatedRow > 0;
	}

	public boolean update(PK id, Map<String, Object> params) {
		params.put("id", id);
		int updatedRow = getSqlSession().update(getKey("uptById"), params);
		return updatedRow > 0;
	}
	...
	public boolean delete(PK id) {
		...
		params.put("field", "id");
		params.put("value", id);
		int deletedRow = getSqlSession().delete(getGlobalKey("del"), params);
		return deletedRow > 0;
	}
	 ...
	 */
	public T get(PK id) {
		return getSqlSession().selectOne(getKey("getFromId"), id);
	}
	...
	public List<T> get(SearchOps search) {
		return getSqlSession().selectList(getKey("getList"), search);
	}
	...
	public List<T> getAll(SearchOps search) {
		return getSqlSession().selectList(getKey("getAllList"), search);
	}
	...
	public Long countAll() {
		HashMap<String, Object> params = new HashMap<String, Object>();
		params.put("model", transferTB(getModelName()));
		return (Long) getSqlSession().selectOne(getGlobalKey("cnt"), params);
	}
	...
	public Long count(SearchOps search) {
		return (Long) getSqlSession().selectOne(getGlobalKey("cnt"), search);
	}
	...
	public <VO> VO get(Class<VO> cls, SearchOps search) {
		return getSqlSession().<VO> selectOne(getGlobalKey("get_" + cls.getSimpleName()), search);
	}
